...
def dashboard():
    ...
    if request.method == "POST":
        ...
        session[
            "selected_account_id"
        ] = account_choice  # Store selected_account_id in the session
        ...
        try:
            ...
            if selected_account:
                ...
                cursor.execute(
                    "SELECT * FROM transactions WHERE from_account_id = %s",
                    (selected_account_id,),
                )
            ...
            current_app.logger.error(
                f"An error occurred while retrieving account details: {e}"
            )
